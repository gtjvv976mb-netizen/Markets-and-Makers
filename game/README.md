...
This is the **v1.0 playable vertical slice** of **Markets & Makers**. Its active open world is **Mercedonia Highlands & Rivers**, with the original B01–B08 Blender structures retained as temporary business placeholders.
...
- Introduced **Merc Dollar (`MERCS`)** as the stable, everyday unit for prices, wages, taxes, leases, utilities, transport, services and upgrades.
...
- Buying reserve `$MM` retires the matching Merc Dollar principal; returning `$MM` issues fewer Merc Dollars, so the spread builds a protection margin.
- Added live reserve coverage, player reserve holdings, Civic Vault, monetary-policy state and Merc Dollars-in-circulation indicators.
...
- Expanded from six to **fifteen playable businesses** across infrastructure, primary production, manufacturing, commerce, Mercedonian services and circular recovery.
...
- Added wages to the Mercedonian spending pool and made maintenance partly technician income, keeping ordinary Merc Dollar transfers conserved.
...
- Switched to a neutral, sRGB-correct display pipeline and added one bounded desktop sunlight shadow pass with cheaper contact shadows on Mercedonians and the player.
...
- Explore Mercedonia Highlands & Rivers through nine connected economic districts in a Three.js world.
...
- Run timestamp-based production or price-sensitive Mercedonian service jobs.
- Buy basic inputs from government infrastructure, sell productive resources to government procurement, and sell services/supplies into Mercedonian demand.
- Pay licenses, payroll, maintenance and a 5% transaction tax in Merc Dollars while reserve conversion separately contracts or expands the monetary base.
- Move optional long-term wealth between Merc Dollars and internal `$MM` reserve holdings at the Civic Reserve Desk.
...
- Maintain equipment condition, build reputation, attract more Mercedonians, and travel between districts.
...
This build uses **prototype Merc Dollars and internal `$MM` reserve holdings only**. It has no wallet connection, Pump.fun integration, token transfer, cash-out, automatic redemption, or promise of returns. The 50 million Merc Dollar opening base is split between government infrastructure/procurement, Mercedonian spending and the player. The 50 million `$MM` Civic Vault is separately accounted as reserve backing.
...
- Active world: `public/assets/world/highlands-rivers-v1/world.gltf`
- Rollback world: `public/assets/world/sunwoven-reach-v1.glb`
...
- `docs/DUAL_CURRENCY_SYSTEM.md` — Merc Dollar balance sheet, `$MM` reserve rules, monetary controls and deployment boundary used by v0.5
