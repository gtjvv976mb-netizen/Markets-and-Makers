/**
 * What a business's floor is worth, as multipliers. THE AUTHORITY'S COPY.
 *
 * A verbatim port of `game/src/floorEffects.ts`. The two packages duplicate rather than share
 * (the same convention as server/src/catalogue.ts), so this file and its client twin must be
 * kept identical BY TEST, not by discipline: `shared/floor-fixtures.json` pins 30 layouts to
 * their expected multipliers and is executed by both suites. If these ever disagree, both go
 * red on the same fixture — which is the only way a spatial rule running in two engines fails
 * loudly instead of silently, in money, in somebody's balance.
 *
 * Everything the rule needs from the client's data module is inlined below. Change one side,
 * change the other, regenerate the fixtures, and watch both suites.
 */
...
export const FITTINGS: Record<FittingKey, {
  serves: UpgradeKey;
  cost: number;
  effect: Partial<{ output: number; speed: number; inputThrift: number; storage: number; price: number }>;
}> = {
  hopper: { serves: "yield", cost: 240, effect: { output: 1.18 } },
  kiln: { serves: "yield", cost: 320, effect: { price: 1.12 } },
  governor: { serves: "speed", cost: 280, effect: { speed: 0.85 } },
  sorter: { serves: "speed", cost: 300, effect: { inputThrift: 0.8 } },
  rack: { serves: "capacity", cost: 200, effect: { storage: 1.4 } },
  counter: { serves: "appeal", cost: 360, effect: { output: 1.1, price: 1.06 } },
};

const FACING_STEP: Record<Facing, { column: number; row: number }> = {
  N: { column: 0, row: -1 },
  E: { column: 1, row: 0 },
  S: { column: 0, row: 1 },
  W: { column: -1, row: 0 },
...
export function apronTiles(
  tile: { column: number; row: number }, facing: Facing, level: number,
