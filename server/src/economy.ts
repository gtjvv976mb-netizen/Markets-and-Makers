...
import {
  ...
  DISTRICT_TRADER_BASELINE, EPOCH_MM_BUDGET, MEAN_REVERSION_CAP, MEAN_REVERSION_PER_MINUTE, MIN_EPOCH_PAYOUT,
  PRESSURE_MAX, PRESSURE_MIN, RESERVE_FUNDING_RATE, RESOURCES, clamp, epochIdFor, utcDay,
} from "./catalogue.js";
...
/** Base budget plus whatever fees and taxes funded the reserve during the previous epoch. */
export async function epochBudget(realmId: string, epochId: number): Promise<number> {
  const funded = await db().query<{ amount: string }>(
    ...
    [realmId, epochId - 1]);
  return EPOCH_MM_BUDGET + Math.floor(Number(funded.rows[0]?.amount ?? 0) * RESERVE_FUNDING_RATE);
}

/** Route a share of a fee or tax into the reserve so emission has a source, not just a balance. */
export async function fundReserve(realmId: string, amount: number, source: string, at = Date.now()): Promise<void> {
  if (amount <= 0) return;
  await db().query(
    `insert into reserve_funding (realm_id, epoch_id, amount, source) values ($1,$2,$3,$4)`,
    [realmId, epochIdFor(at), Math.floor(amount), source]);
}
